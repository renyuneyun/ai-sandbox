...
46. **Git worktree auto-mount:** in a linked worktree (`.git` file with `gitdir: <path>`), the common git dir is mounted read-write at the same path. Verify: `git status`/`git log` work and `git commit` succeeds inside the sandbox. In a normal checkout (`.git` is a dir) nothing extra is mounted.
